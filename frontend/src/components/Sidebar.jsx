...
const icons = {
  ...
  trail: <><path d="M5 19c4-5 10-4 14-14" /><circle cx="5" cy="19" r="2" /><circle cx="19" cy="5" r="2" /><path d="M11 11l2 2" /></>,
  subjects: <><path d="M4 6.5 12 3l8 3.5L12 10 4 6.5Z" /><path d="M6 9v5.5c3 2 9 2 12 0V9" /><path d="M20 7v6" /></>,
...
function Icon({ name }) {
  return <svg viewBox="0 0 24 24" className="h-[17px] w-[17px] shrink-0" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">{icons[name]}</svg>;
}

function Item({ to, label, icon, active, onClick }) {
  return (
    <Link to={to} onClick={onClick} className={`sidebar-link ${active ? "sidebar-link-active" : ""}`}>
      <Icon name={icon} />
      <span>{label}</span>
    </Link>
  );
...
export default function Sidebar({ open, onClose }) {
  ...
  useEffect(() => {
    if (location.pathname.startsWith("/subjects")) setSubjectsOpen(true);
  }, [location.pathname]);
  ...
  return (
    <>
      {open && <button aria-label="Close learning menu" onClick={close} className="fixed inset-0 z-40 bg-ink/25 backdrop-blur-sm lg:hidden" />}
      <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
        <div className="flex items-center justify-between lg:hidden">
          <p className="font-display text-lg font-semibold">Learning menu</p>
          <button onClick={close} className="rounded-lg p-2 text-slate hover:bg-ink/5" aria-label="Close menu">✕</button>
        </div>
        ...
        <div className="mt-4 space-y-1">
          ...
          <button type="button" onClick={() => setSubjectsOpen(v => !v)} className={`sidebar-link w-full justify-between ${active("/subjects") ? "sidebar-link-active" : ""}`}>
            <span className="flex items-center gap-3"><Icon name="subjects" /><span>Subjects</span></span>
            <span className={`text-xs transition-transform ${subjectsOpen ? "rotate-180" : ""}`}>⌄</span>
          </button>

          {subjectsOpen && (
            <div className="ml-4 border-l border-ink/10 pl-3 py-1 space-y-1 animate-fade-up">
              {subjects.map(subject => {
                const slug = subject.toLowerCase().replace(/\s+/g, "-");
                return (
                  <Link key={subject} to={`/subjects/${slug}`} onClick={close} className={`sidebar-subject ${location.pathname === `/subjects/${slug}` ? "sidebar-subject-active" : ""}`}>
          ...
          <Item to="/doubt-solver" label="Doubt Solver" icon="doubt" active={active("/doubt-solver")} onClick={close} />
        ...
        <div className="mt-6 rounded-2xl bg-ink p-4 text-paper shadow-lg shadow-ink/10">
          <div className="flex items-center gap-2"><span className="grid h-6 w-6 place-items-center rounded-full bg-gold text-xs text-ink">✦</span><span className="text-[9px] font-mono uppercase tracking-[.16em] text-gold">Trailhead insight</span></div>
          <p className="mt-3 font-display text-lg font-semibold">Your route keeps adapting.</p>
          <p className="mt-1 text-[11px] leading-5 text-paper/55">Use subject quizzes and the doubt solver to give your trail better information.</p>
          <Link to="/analytics" onClick={close} className="mt-3 inline-flex text-xs font-semibold text-gold hover:text-paper">See progress →</Link>
        </div>
        ...
        <div className="mt-auto pt-6">
          ...
          <div className="mt-4 flex items-center gap-3 rounded-2xl border border-ink/10 bg-white p-3">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-gold font-display font-semibold text-ink">{user?.name?.charAt(0)?.toUpperCase() || "T"}</span>
            <div className="min-w-0"><p className="truncate text-xs font-semibold">{user?.name || "Student"}</p><p className="text-[10px] font-mono uppercase tracking-wider text-slate">{user?.studentClass || "Learner"}</p></div>
          </div>
        </div>
      </aside>
    </>
